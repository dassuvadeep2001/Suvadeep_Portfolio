...
const Hobbies = () => {
  return (
    ...
    <div className="w-full px-6 md:px-16 py-12 mt-8">
      ...
      <div className="flex justify-center items-center w-full md:h-[60vh] h-full">
        <Swiper
          effect={'coverflow'}
          grabCursor={true}
          centeredSlides={true}
          slidesPerView={3} // Shows previous and next images
          spaceBetween={20}
          coverflowEffect={{
            rotate: 30,
            stretch: 0,
        ...
        >
          {images.map((src, index) => (
            <SwiperSlide key={index} className="shadow-xl shadow-gray-500 overflow-hidden flex justify-center my-10">
              <img src={src} alt={`Drawing ${index + 1}`} className="h-auto w-auto max-w-full max-h-full object-contain" />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
     ...
     <motion.p
       className=" text-gray-600 mb-6 text-center max-w-2xl font-semibold"
       initial={{ opacity: 0 }}
       animate={{ opacity: 1 }}
       transition={{ delay: 0.3, duration: 0.5 }}
     >
     ...
     <div className='md:w-[100vh] w-full'>
     <div className="grid grid-cols-3 md:gap-4 gap-2 max-w-6xl">
